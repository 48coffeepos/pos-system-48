  ...
  .handler(async ({ data }) => {
    try {
      ...
      const menu = await prisma.$transaction(async (tx) => {
        ...
        if (!data.trackInventory) {
          return tx.menu.findUnique({
            where: { menu_id: resolvedMenuId },
            include: { menuInventories: { include: { inventory: true } } },
          });
        }
        ...
        if (data.itemType === Inventory_Type.CUP) {
          const cupLinks = data.selectedCupIds
            ...
            .filter((entry): entry is { cupId: string; price: number } =>
      ...
      return {
        ...
        type: menu.type,
        menuInventories: menu.menuInventories.map((menuInventory) => ({
