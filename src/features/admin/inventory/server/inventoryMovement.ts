...
export type InventoryMovement =
	...
	| { kind: "sale_reversal"; quantity: number }
	| { kind: "beginning_sale"; quantity: number }
	| { kind: "beginning_sale_reversal"; quantity: number };
...
export async function applyInventoryMovement(
	...
	switch (movement.kind) {
		...
		case "sale_reversal":
			if (item.type === "SUPPLIES") {
				throw new Error(
					"SUPPLIES inventory items are not restored via sale reversal.",
				);
			}
			data = {
				out_store: { decrement: Math.min(quantity, item.out_store) },
				ending_store: storeEnding + quantity,
			};
			break;
		case "beginning_sale":
			...
			data = {
				beginning_store: { decrement: quantity },
				ending_store: storeEnding - quantity,
			};
			break;
		case "beginning_sale_reversal":
			if (item.type === "SUPPLIES") {
				throw new Error(
					"SUPPLIES inventory items are not restored via beginning sale reversal.",
				);
			}
			data = {
				beginning_store: { increment: quantity },
				ending_store: storeEnding + quantity,
			};
			break;
		default: {
