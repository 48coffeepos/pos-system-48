  ...
  .handler(async ({ data }) => {
    ...
    if (order.method === "GRAB" && data.method !== "GRAB") {
      ...
      for (const item of order.order_items) {
        let itemLineTotal = 0;
        if (!item.loyalty) {
          itemLineTotal = (Number(item.unit_price) * item.quantity) - Number(item.discount_amount || 0);
        }
        
        let addonsTotal = 0;
        for (const addon of item.addon_items) {
          const addonTotalPrice = Number(addon.addon_price_snapshot) * addon.quantity;
