...
export function PosReceiptDialog({
  ...
  const cashierName = order?.cashier_name?.trim() || "Cashier";
      ...
      <AlertDialogContent className="w-[calc(100vw-1rem)] max-w-[380px] border-(--light-gray) bg-(--pure-white) p-0">
        <div className="flex flex-col max-h-[85vh] p-3 sm:p-6">
          <Button
            ...
            onClick={onClose}
            className={cn(
              "absolute top-2 right-2 rounded-full md:top-4 md:right-4",
              posBtnGhost,
            )}
            aria-label="Close"
          >
          ...
          <div className="flex-1 overflow-y-auto min-h-0 scrollbar-none">
            ...
            >
                    ...
                    {item.addon_items && item.addon_items.length > 0 ? (
                      <div className="mt-0.5 grid grid-cols-[35px_1fr_60px] text-[9px] md:grid-cols-[45px_1fr_70px] md:text-[11px]">
                        ...
                        <span className="wrap-break-word whitespace-normal">
                          ...
                          {item.addon_items
                            .map(
                              (a) => `${a.addon_name_snapshot} x${a.quantity}`,
                            )
                            .join(", ")}
                        </span>
                        <span />
              ...
              <div className="mt-3 text-center md:mt-6">
                <p className="text-[11px] font-black uppercase md:text-[13px]">
                  {cashierName}
                </p>
          ...
          <div className="no-print flex gap-2 pt-2 bg-(--pure-white) md:gap-3 md:pt-3">
            <Button
              ...
              onClick={onClose}
              className={cn(
                "h-8 flex-1 text-[9px] md:h-12 md:text-sm",
                posBtnOutline,
              )}
            >
              Save
            ...
            <Button
              onClick={() => handlePrint()}
              className={cn(
                "flex h-8 flex-1 gap-1 text-[9px] md:h-12 md:gap-2 md:text-sm",
                posBtnPrimary,
              )}
            >
